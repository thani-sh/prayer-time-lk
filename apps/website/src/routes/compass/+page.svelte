<script lang="ts">
	 ...
	 *
	 * Sources:
	 * - Location: Geolocation API. Falls back to Colombo coordinates
	...
	const FALLBACK = { lat: 6.9271, lon: 79.8612, name: 'Colombo' };
	...
	function useFallback() {
		...
		locationNote = `Approximate direction from ${FALLBACK.name}. Enable location services for better accuracy.`;
	}
...
<div class="flex flex-col items-center justify-center min-h-svh gap-5 px-6 pb-28">
	{#if pageState === 'permission'}
		<div class="flex flex-col items-center gap-4 text-center max-w-xs">
			<p class="text-lg font-medium">The compass needs access to your phone's sensors</p>
			<p class="text-sm opacity-60">Tap the button, then rotate your phone to face the Qibla.</p>
			<button class="btn btn-primary btn-lg" onclick={enableCompass}>Enable compass</button>
		</div>
	{:else}
		...
		<div class="text-center">
			...
			{#if qibla !== null}
				...
				<p class="text-xs opacity-50 mt-1">Rotate until the arrow points straight up</p>
			{/if}
			{#if locationNote}
